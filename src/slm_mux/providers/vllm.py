"""
slm_mux.providers.vllm -- Local vLLM server provider.

Routes chat-completion requests to vLLM servers registered in a JSON
registry file (typically ``slurm/vllm_servers.json``).  The servers
...
from slm_mux.providers.registry import load_registry, resolve_model
...
class VLLMProvider(ProviderBase):
    """
    Provider for local vLLM OpenAI-compatible inference servers.
    ...
    registry_path:
        ...
        ``slurm/vllm_servers.json`` under the project root.
    registry:
        Optional pre-loaded registry dict (overrides *registry_path*).
    registry_type:
        Registry type string passed to :func:`load_registry`.
    max_retries:
    ...
    def _load_registry_with_fallbacks(self) -> Dict[str, Dict[str, Any]]:
        ...
        for fb_path in self._fallback_paths:
            ...
            reg = load_registry(path=fb_path, registry_type=self._registry_type)
            if reg:
                return reg
    ...
    def reload_registry(
        ...
        Useful after new vLLM servers are started or old ones are
        decommissioned.
        """
        rtype = registry_type or self._registry_type
        self._registry = load_registry(
            path=path or self._registry_path, registry_type=rtype
        )
        logger.info(
            "Reloaded vLLM registry (%d model(s))", len(self._registry)
        )
    ...
    def _resolve_endpoint(self, model_name: str) -> Tuple[str, str]:
        ...
        if entry is None:
            ...
            raise RuntimeError(
                f"Model {model_name!r} not found in vLLM registry. "
    ...
    def _request_once(
        ...
        """Issue one HTTP POST to the vLLM server."""
        ...
        if not resp.is_success:
            ...
            try:
                ...
                pass
            logger.error(
                "[VLLMProvider] HTTP %d for model=%s url=%s: %s",
                resp.status_code,
                model_name,
                chat_url,
                body[:500],
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        top_k:
            Top-k sampling parameter (supported by vLLM).
        top_logprobs:
