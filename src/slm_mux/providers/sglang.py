"""
slm_mux.providers.sglang -- SGLang OpenAI-compatible provider.

Routes chat-completion requests to SGLang servers registered in a JSON
registry file (typically ``slurm/sglang_servers.json``).  The servers
...
class SGLangProvider(ProviderBase):
    """
    Provider for local SGLang OpenAI-compatible inference servers.
    ...
    registry_path:
        ...
        ``slurm/sglang_servers.json`` under the project root.
    registry:
    ...
    def reload_registry(
        ...
        Useful after new SGLang servers are started or old ones are
        decommissioned.
        """
        ...
        logger.info(
            "Reloaded SGLang registry (%d model(s))", len(self._registry)
        )
    ...
    def _resolve_endpoint(self, model_name: str) -> Tuple[str, str]:
        ...
        if entry is None:
            ...
            raise RuntimeError(
                f"Model {model_name!r} not found in SGLang registry. "
    ...
    def _request_once(
        ...
        """Issue one HTTP POST to the SGLang server."""
        ...
        if not resp.is_success:
            ...
            try:
                ...
                pass
            logger.error(
                "[SGLangProvider] HTTP %d for model=%s url=%s: %s",
                resp.status_code,
                model_name,
                chat_url,
                body[:500],
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        """
        Send a chat-completion request to a local SGLang server.
